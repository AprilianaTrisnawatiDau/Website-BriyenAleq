...
$totalData = 0;
if($conn){
    $resPenjual = mysqli_query($conn, "SELECT nama as penjual, COUNT(*) as jumlah FROM penjualan GROUP BY nama");
...
.sidebar .ornament img {
  ...
  margin: 20px auto 0;
  opacity: 0.3;
  filter: drop-shadow(0 0 10px rgba(20, 174, 92, 0.4));
}
...
<div class="dashboard-container">
  ...
  <main class="main-content">
    ...
    <section class="welcome">
      ...
      <img src="../images/logoweb.png" alt="Logo BriyenAleq" class="main-logo" />
    </section>

    <section class="widgets">
      <div class="widget-grid">
        <?php foreach($penjualData as $p): ?>
          <div class="widget">
            <h2><?= $p['jumlah'] ?></h2>
            <p>Produk Terjual: <?= htmlspecialchars($p['penjual']) ?></p>
          </div>
        <?php endforeach; ?>
        <div class="widget">
          <h2><?= $totalData ?></h2>
          <p>Total Semua Produk Terjual</p>
        </div>
      </div>
    </section>
  </main>
